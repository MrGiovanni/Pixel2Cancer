...
def generate_tumor(img, mask, steps, kernel_size, organ_standard_val, organ_hu_lowerbound, outrange_standard_val, threshold, organ_name):
    
    steps = np.random.randint(10, steps)
    print(steps)
    ...
    current_state = torch.tensor(
        processed_organ_region, dtype=torch.int32).cuda('cuda:0')
    density_organ_state = torch.tensor(
        density_organ_map, dtype=torch.int32).cuda('cuda:0')
    ...
    mask_tumor[min_x:max_x+1, min_y:max_y+1, min_z:max_z+1] = tumor_out[step]
