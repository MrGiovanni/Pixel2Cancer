...
__device__ int check_tissue(
    ...
    // seprate tissue area into 3 level
    ...
    // probability of grow in tissue area
...
__device__ int mass_effect(
    ...
    // check the density of current tissue
    if (curr_val < threshold * 10/10){
        return 1;
    }// check the density of target tissue
    else {
        return 0;
    }
}
