...
def update_cellular(state_tensor, density_state_tensor, ranges, thresholds, grow_per_cell=1, max_try=-1):
    if max_try < 0:
        max_try = grow_per_cell * 3
    
    return _CellularUpdate.apply(ranges, grow_per_cell, max_try, state_tensor, density_state_tensor, thresholds)
...
class _CellularUpdate(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        _C.update_cellular(state_tensor, density_state_tensor, Y_range, X_range, Z_range, grow_per_cell, max_try, organ_hu_lowerbound, organ_standard_val, outrange_standard_val, threshold, state_tensor_new)
